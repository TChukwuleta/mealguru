...
const pinVerification = catchAsync(async (req, res) => {
  try {
    let user = await authService.getUserById(req.user._id);
    if (req.body.pin != user.user.pin) {
      throw new ApiError("400", "Invalid Pin entered");
    }
    user = await authService.emailVerification(req.user.email);
    res.send({
      message: "Account activated successfully",
      user,
    });
...
const getVendors = catchAsync(async function (req, res) {
  const options = pick(req.query, ["sortBy", "limit", "page"]);
  const { users, page } = await authService.fetchUsers(
    JSON.parse(JSON.stringify({ type: "VENDOR" })),
    options
  );
  const count = await authService.count({ type: "VENDOR" });
  res.status(200).send({
    status: "success",
